...
class LibrariesManager:
    ...
    def optimize(self):
        ...
        for alg, libs_names in libs_data.items():
            ...
            # drop slow libs
            self.libs[alg] = [
                lib for lib in libs if [lib.module_name, lib.func_name] in libs_names
            ]
            # sort libs by speed
            self.libs[alg].sort(
                key=lambda lib: libs_names.index([lib.module_name, lib.func_name])
            )
...
class LibraryBase:
    ...
    def __init__(
        self, module_name, func_name, attr=None, presets=None, conditions=None
    ):
...
prototype.register(
    "DamerauLevenshtein", LibraryBase("abydos.distance", "DamerauLevenshtein")
)
prototype.register(
    "DamerauLevenshtein",
...
prototype.register(
    "DamerauLevenshtein", TextLibrary("jellyfish", "damerau_levenshtein_distance")
)
...
prototype.register(
    "JaroWinkler",
    TextLibrary(
        "jellyfish", "jaro_winkler_similarity", conditions=dict(winklerize=True)
    ),
)
